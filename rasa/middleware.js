...
// see https://medium.com/@harjun1601/building-a-chatbot-with-botkit-and-rasa-a18aa4d69ebb
// this middleware enables botkit to use rasa when it receives a new request
...
module.exports = function(config) {
  ...
  let middleware = {
    receive: function(bot, message, next) {
      // console.log("middleware.receives", message);
          ...
          if (body) {
            ...
            if (bodyEntities) {
              for (let i = 0; i < bodyEntities.length; i++) {
                if (
                  bodyEntities[i].entity in messageEntities &&
    ...
    hears: function(patterns, message) {
      // console.log("middleware.hears", patterns, message);
      if (patterns.indexOf("*") >= 0) {
        return true;
      }

      for (let t = 0; t < patterns.length; t++) {
        if (message.intent && message.intent.name === patterns[t]) {
          return true;
        }
      }
    },
